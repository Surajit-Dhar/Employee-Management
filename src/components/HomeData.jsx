...
const HomeData = () => {
  ...
  return (
    ...
    <div>
      <h1 style={{margin:"30px 40%", color:"#1e3ba4"}}>Employee's List</h1>
      ...
      <Button color="primary" variant="contained" onClick={()=> navigate("/add-employee")}>Add Data</Button>
        ...
        <TableBody>
          { cities && cities.filter(ci => ci.city.toLowerCase().includes(filter)).map((elem) => (
            <StyledTableRow key={elem.id}>
              ...
              <StyledTableCell align="center">{elem.population}</StyledTableCell>
              <StyledTableCell align="center">{elem.age}</StyledTableCell>
              <div className={buttonSty.root}>
              <div style={{marginLeft:"100px"}}>
              <ButtonGroup variant="contained" aria-label="contained primary button group">
                <Button color="primary" style={{marginRight:"10px"}} onClick={()=> navigate(`/edit-employee/${elem.id}`)}>Edit</Button>
